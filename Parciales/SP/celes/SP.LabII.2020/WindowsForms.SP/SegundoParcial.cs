...
namespace WindowsForms.SP
{
    ...
    public partial class SegundoParcial : Form
    {
        ...
        {
            MessageBox.Show("Gabor Roberta");
            this.Text = "Gabor Roberta";
        }
        ...
        private void btnPunto6_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string host = @"Data Source=DESKTOP-Q90H3PF;Database=sp_lab_II_utiles;Trusted_Connection=True";
                ...
                this.da.SelectCommand = new SqlCommand("SELECT id, marca, precio, tipo FROM sp_lab_II_utiles", cn);//como el default esta en master lo pongo completo
        ...
        private void btnPunto7_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (Goma p in c_gomas.Elementos)//recorro cada lista para sacar goma/lapiz/utiles
                {
                    fila = this.dt.NewRow();
                    ...
                    fila[1] = p.marca;
                    fila[2] = p.precio;
                    fila[3] = p.GetType().Name;

                    this.dt.Rows.Add(fila);
                }
                foreach (Lapiz p in c_lapices.Elementos)
                {
                    fila = this.dt.NewRow();
                ...
                foreach (Utiles p in c_utiles.Elementos)
                {
                    fila = this.dt.NewRow();
        ...
        {
            try//try en todo lo que quiera acceder a la base
            {
                //Configurar el updateCommand del SqlDataAdapter y sus parámetros
                this.da.UpdateCommand = new SqlCommand("UPDATE [sp_lab_II_utiles].[dbo].[utiles] SET marca=@marca, precio=@precio, tipo=@tipo WHERE id=@id", cn);
                ...
                //Modificar el último registro del dataTable//marca:barrilito; precio:72
                int indice = dt.Rows.Count - 1;
